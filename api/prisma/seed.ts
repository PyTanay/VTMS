...
async function main() {
  ...
  const usersToSeed = [
    { username: 'admin', email: 'admin@gnfc.in', role: Role.ADMIN },
    { username: 'employee', email: 'employee@gnfc.in', role: Role.RECOMMENDING_EMPLOYEE },
    { username: 'sectionhead', email: 'head@gnfc.in', role: Role.TRAINING_CENTER_SECTION_HEAD },
    { username: 'incharge', email: 'incharge@gnfc.in', role: Role.TRAINING_IN_CHARGE },
    { username: 'approver', email: 'approver@gnfc.in', role: Role.ED_GM_APPROVER },
  ];
  ...
  for (const emp of sampleEmployees) {
    ...
    if (emp.employee_no === '8979') {
      await prisma.user.upsert({
        ...
        create: { username: 'admin', email: 'admin@gnfc.in', password: defaultPasswordHash, role: Role.ADMIN, employeeId: dbEmp.id }
      });
    } else if (emp.employee_no === '1001') {
      await prisma.user.upsert({
        where: { username: 'employee' },
        update: { employeeId: dbEmp.id },
        create: { username: 'employee', email: 'employee@gnfc.in', password: defaultPasswordHash, role: Role.RECOMMENDING_EMPLOYEE, employeeId: dbEmp.id }
      });
    }
  }
